...
export default function HeaderComponent() {
  ...
  return (
    <div className="sticky top-0 flex justify-around items-center bg-[var(--color-bg)] h-24 sm:h-18 flex-wrap px-12 sm:px-0">
      ...
      <BurgerIcon
        className="absolute right-0 w-12 h-12 sm:hidden text-[var(--color-text)] hover:cursor-pointer"
...
function ModalMenu({ propOpenModalMenu }: { propOpenModalMenu: (value: boolean) => void }) {
  const modalContent = (
    ...
    >
      <div
        className="absolute right-0 top-12 sm:top-9 h-fit w-1/2 max-w-xs bg-[var(--color-bg)] shadow-lg p-6 flex flex-col gap-4 rounded-l-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <nav className="flex flex-col gap-4 font-semibold [&>*]:p-2 [&>*]:text-[var(--color-text)] [&>*]:hover:text-blue-400 [&>*]:hover:underline">
          {LIST_PAGES.map((obj) => (
